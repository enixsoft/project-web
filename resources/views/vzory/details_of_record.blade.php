...
@if(Auth::user()->zamestnanec_id == $premenna0 || Auth::user()->role == "admin" )
      ...
      <form class="form-horizontal" method="POST" action="{{ $controller }}"> 				
        <table class="table">
            ...
            <tr>
                <th><h4>{{$stlpec2}}</h4></th>
                <td><textarea class="form-control" name="textarea1" rows="7">{{ object_get($z, $premenna2) }}</textarea></td>
            </tr>
            <tr>
                <th><h4>{{$stlpec3}}</h4></th>
                <td><textarea class="form-control" name="textarea2"  rows="7">{{ object_get($z, $premenna3) }}</textarea></td>
            </tr>
            @endforeach
        </table>
